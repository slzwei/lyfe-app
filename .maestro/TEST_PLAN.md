...
**18/18 flows passing on main** (4 smoke + 6 role + 3 lead + 5 candidate). Phases 0 + 1 + 2a + 4a + 4b (partial) + 4c (partial) complete. Lead pipeline has end-to-end coverage of status transitions (full + smoke), search/filter, and the agent-personal-only RLS contract. Candidate coverage now includes detail (manager + PA), status transition (manager via native iOS ActionSheet), interview scheduling (manager via Modal — Phase 4a's `accessibilityViewIsModal` mitigation held), and a PA-without-manager negative path (UI fallback assertion that validates the RLS boundary without invoking RLS directly). Three lead flows (call, whatsapp, reassign) deferred to Phase 2b. Three candidate flows (create-from-staff, create-from-pa, upload-document) deferred to Phase 4b-tail — KeyboardAwareScrollView + iOS-26 + Maestro form-input wall plus document picker bypass not yet implemented. One candidate flow (realtime-disc-complete) deferred to Phase 4c-tail — the candidate detail screen doesn't subscribe to `progress_signals` (only the candidates list does, via `useCandidatePipeline` → `useCandidateRealtime`), so an UPDATE mid-flow has nothing to refresh; needs a product feature add plus Maestro `runScript`/admin-auth infra. Real users with VoiceOver are unaffected — `accessibilityViewIsModal` + explicit modal-button labels are in place.
...
**Headline**: ~18% coverage. All 6 roles + lead status/search/agent-only RLS contract + candidate detail (staff + PA) + candidate status transition + interview scheduling + PA-without-manager negative path. Seven flows total deferred (3 lead modal-from-detail in 2b; 3 candidate create + upload in 4b-tail; 1 realtime-disc-complete in 4c-tail).
...
## Coverage limitations — honest read of what 18/18 means

The 18 flows are a real safety net for **login + role + read** paths, a thin slice of the lead pipeline, and the candidate-detail / mutation / negative-path edges. They are **not** a comprehensive safety net. Areas not covered:
