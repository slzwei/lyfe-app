...
| 36 | events RLS + events/create.tsx | Candidate | **PARTIALLY FIXED 2026-07-08** (calendar overhaul, #44): /(tabs)/events/create now has a useRequireRole('admin','director','manager','pa','ro') guard mirroring the leads/candidates layouts — the deep-link hole is closed at the route level. STILL OPEN (RLS half): `authenticated_insert_events` is `WITH CHECK (auth.uid() IS NOT NULL)` — any candidate can INSERT events at API level (verified in prod pg_policy 2026-07-03) — and `read_events` / `read_event_attendees` `USING(true)` expose the full agency calendar + attendee rows to any authenticated candidate. Needs an RLS migration (separate security workstream). | P2 |
