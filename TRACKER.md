...
| 36 | events RLS + events/create.tsx | Candidate | `authenticated_insert_events` is `WITH CHECK (auth.uid() IS NOT NULL)` — any candidate can INSERT events (verified in prod pg_policy 2026-07-03) — and /(tabs)/events/create has no useRequireRole guard (the hidden "+ New" button is UI-only; the route is deep-linkable). `read_events` / `read_event_attendees` `USING(true)` also expose the full agency calendar (57 events incl. coordinates) + all 227 attendee rows to any authenticated candidate at API level (verified via rolled-back impersonation). | P2 |
