...
import { MOCK_CANDIDATES } from '@/app/(tabs)/candidates/index';
...
export default function PaScreen() {
    ...
    const loadCandidates = useCallback(async () => {
        ...
        if (!user?.id) return;
        setError(null);
        const { data, error: fetchError } = await fetchCandidates(user.id, false);
        if (fetchError) {
            setError(fetchError);
        } else {
            setCandidates(data);
        ...
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            <ScreenHeader
                ...
                rightAction={
                    <TouchableOpacity
                        ...
                        onPress={() => router.push('/(tabs)/pa/add-candidate' as any)}
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <CandidateCard
                        candidate={item}
                        onPress={() => router.push(`/(tabs)/pa/candidate/${item.id}` as any)}
                    />
                )}
            />
        </SafeAreaView>
