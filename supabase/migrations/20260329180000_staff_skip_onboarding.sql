...
BEGIN
  ...
  -- Staff roles skip onboarding (it's candidate-specific)
  IF v_role IN ('admin', 'director', 'manager', 'agent', 'pa') THEN
    v_onboarding := true;
  END IF;

  -- 2. Insert user first (so FK on accepted_by_id works)
  ...
  VALUES (
    ...
    v_manager_id,
    v_onboarding,
    v_onboarding  -- staff skip email verification too
  )
  ON CONFLICT (id) DO UPDATE SET
    last_login_at = now(),
    phone         = COALESCE(EXCLUDED.phone, users.phone);

  -- 3. Mark invitation accepted after user exists
